...
def generation(config, datapackage_dir):
    ...
    carrier = pd.read_csv(os.path.join(datapackage_dir,'archive/carrier.csv'),
                          index_col=[0,1]).loc[('base', config['year'])]
...
def hydro_generation(config, datapackage_dir):
    ...
    ror_shares = pd.read_csv(os.path.join(datapackage_dir, 'archive', 'ror_ENTSOe_Restore2050.csv'),
